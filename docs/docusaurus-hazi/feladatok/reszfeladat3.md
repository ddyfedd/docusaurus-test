...
Ebben a részben egy egyszerű CI/CD (Continuous Integration/Continuous Deployment) folyamatot fogsz beállítani GitHub Actions segítségével, hogy a Docusaurus oldalad automatikusan buildelődjön és publikálódjon GitHub Pages-re.
...
module.exports = {
  ...
  baseUrl: '/<REPOSITORY_NEVE>/', // Cseréld le a repository nevére!
                                     // Ha a repository neve <FELHASZNALONEVED>.github.io, akkor a baseUrl '/' legyen.
...
*Segítség:* [Docusaurus Deployment - GitHub Pages](https://docusaurus.io/docs/deployment#deploying-to-github-pages)
...
1.  **Workflow fájl létrehozása (`deploy.yml`):**
    ...
    * Ha az API dokumentáció generált fájljait nem commitolod (ajánlott nem commitolni őket, és a CI/CD-vel generáltatni), akkor a `Generate API Docs` lépés szükséges. Ha commitolod őket, ez a lépés kihagyható. A házi feladat során korábban commitoltuk őket, de a CI/CD során jobb, ha a forrásból (OpenAPI spec) generálódnak - ebben az esetben az output mappát hozzá is adhatod a `.gitignore`-hoz, hogy ne zavarjon fejlesztés közben a sok új fájl, ami a generáláskor létrejön. Döntsd el, melyik utat követed, és szükség szerint módosítsd a workflow-t. Ebben a példában benne hagytam a generálást.
    :::

1.  **Repository Beállítások GitHub Actions-höz:**
...
*Segítség:* [Docusaurus Deployment - Triggering deployment with GitHub Actions](https://docusaurus.io/docs/deployment#triggering-deployment-with-github-actions)
...
1.  **Deployment ellenőrzése:**
    ...
    * Ha sikeres volt, a GitHub repository **Settings -> Pages** részében látnod kell, hogy az oldalad publikálva lett a `gh-pages` branch-ről. Az URL-nek meg kell jelennie (pl. `https://<FELHASZNALONEVED>.github.io/<REPOSITORY_NEVE>/`).
...
2.  **Commit és push (egy új branch-en tesztelve):**
    ...
    * Ezt a teszt PR-t nem szükséges merge-elni, csak a workflow működésének ellenőrzésére szolgál. Ha működik, a `test.yml` fájlt hozzáadhatod a `main` branch-hez egy külön PR-rel.
